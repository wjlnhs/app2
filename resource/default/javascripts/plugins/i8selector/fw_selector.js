...
define(function (require, exports, module) {
    //require('/default/javascripts/plugins/i8tree/js/themes/default/style.css');
    require('/default/javascripts/plugins/i8scrollbar/css/mscrollbar.css');
    require('./css/fw_i8selector.css');
    //require('/default/javascripts/plugins/i8tree/js/jstree.js');
    require('/default/javascripts/plugins/i8scrollbar/mscrollbar.js');
    require('/default/javascripts/common/underscore-min-cmd');
    exports.KSNSelector = function (setting) {
        if (cpec_session.avip > 0) {/*VIP用户*/
        ...
        setting = $.extend({ model: 1, element: null, width: 250, maxSelected: 100, isShowQuit: false, readonly: false, showIco: false, isAbox: false, searchType: { "org": false, "user": true, "grp": false }, loadItem: { items: [], loadedCallBack: function (obj) { } }, selectCallback: function () { }, deleteCallback: function () { } }, setting);
        var singleObj = null;
        /*根据ID初始化控件*/
        var initSelector = function (elemid) {
            ...
            var opts2 = {
                ...
                url: "/webajax/plugins/selector/selectentity?r=t",
                model: setting.model,
            ...
            if (setting.model == 0) {
                select2.init();
                select2.seletedCallback = function (uid, uname, uemail) {
                    return setting.selectCallback(uid, uname, uemail);
                }
            } else {
                select2.KIInit();
    ...
    (function ($) {
        $.extend({
            ...
            "includePath": cpec_session.resHost,
            ...
            "detectAboxer": function (obj, setting) {
                ...
                keybaordIco.click(function () {
                    ...
                    var advBoxer = $('<div class="fw_boxerContainer"><div class="bc_loading"><img src="'+cpec_session.resHost+'default/images/16x16loading.gif"/><span>加载中...</span></div></div>').click(function (e) { _ksnbox.find(".fw_ksnplaceholder").hide(); return false; });
                    ...
                    var boxer_top = $('<div class="bcon_top"></div>').append($('<a title="关闭" class="i8selector-popu-closed">x</a>').click(function () { advBoxer.remove(); }));
                    var boxer_ctr = $('<div class="bcon_center"><div class="bcc_left"><div class="bcl_srchbox" style="display:none"><input type="text" class="searchkeyword" placeholder="姓名、拼音可检索"/><span class="span_act_search"></span><span class="span_act_delete"></span></div><div class="bcl_list"><div class="list_tab">' + tabHtml + '</div><div id="tab_list_container" class="list_details"><div id="tbls_org_box" style="display:none" class="tb_refence_box" dataload="0"><ul id="cropOrgTree" class="ztree">加载中...</ul></div><div id="tbls_grp_box" style="display:none" class="tb_refence_box" dataload="0">加载中...</div><div id="tbls_clg_box" style="display:none" class="tb_refence_box" dataload="0">加载中...</div></div></div></div><div class="bcc_ctr"><input type="button" id="btn_addselecteditemforls" class="btn_add_item disable_add"/></div><div class="bcc_right"><div id="bccr_selectedItem_container" class="bccr_selectedItem"><div id="con_selectorItemList"></div></div></div></div>');
                    ...
                    /*添加到DOM*/
                    advBoxer.empty().append(boxer_top).append(boxer_ctr).append(boxer_ftr);
                    /*阻止单击冒泡并绑定单击事件*/
                    ...
                    if (ksnSelector.find(".fw_ksninput_slted").length > 0) {
                        ...
                        ksnSelector.find(".fw_ksninput_slted").each(function () {
                            var jqItem = $(this);
                            span_itemHtml += '<span class="item_slted" tagid="' + jqItem.attr("data-uid") + '" tagname="' + jqItem.text() + '" tagtype="' + jqItem.attr("data-type") + '"><em>' + jqItem.text() + '</em><a></a></span>';
                        });
                    ...
                    $(".btn_submit_select", advBoxer).click(function () {
                        ...
                        if (_selecteditems.length == 0) {
                            ...
                            for (var i = 0; i < _selecteditems.length; i++) {
                                ...
                                span_itemHtml += "<span class=\"fw_ksninput_slted\" data-uid=\"" + jqItem.attr("tagid") + "\" data-type=\"" + jqItem.attr("tagtype") + "\"><em>" + comName + "</em><a></a></span>";
                                //}
                            }
                            ...
                            if (modelset == 1) {
                                ...
                                if (setting.selectCallback) {
                                    setting.selectCallback(_selecteditems);
                                }
                            }
                        }
                    });
                    ...
                    function convertTreeJson(source) {
                        ...
                        for (n in source) {
                            var item = source[n];
                            if (item.ID == item.RootOrgID) {
                                parent = item.ID;
                            }
                            if (!tmp[item.ID]) {
                                tmp[item.ID] = {};
                            }
                            tmp[item.ID].text=item.OrgName;
                            tmp[item.ID].name = item.OrgName;
                            tmp[item.ID].ID = item.ID;
                            tmp[item.ID].isParent = true;
                            tmp[item.ID].RootOrgID = item.RootOrgID;
                            tmp[item.ID].isExpand = false;
                            tmp[item.ID].nodeType = treeNodeEnable.org ? "org" : "none";
                            if (!("children" in tmp[item.ID])) tmp[item.ID].children = [];
                            if (item.ID != item.ParentOrgID) {
                                if (tmp[item.ParentOrgID]) {
                                    tmp[item.ParentOrgID].children.push(tmp[item.ID]);
                                }
                                else {
                                    tmp[item.ParentOrgID] = { children: [tmp[item.ID]] };
                                }
                            }
                        }
                        return tmp[parent];
                    ...
                    var loadOrgTree = function () {
                        $.get("/platform/getorgdata?rd=" + Math.random(), {}, function (response) {
                            if (response.Result) {
...
/*                                $('#tbls_org_box').jstree({
                    ...
                    var loadGrpList = function () {
                        $.get("/platform/getmygroups", { fn: "gt", range: "pub", p: 1, s: 300, state: "0", orderby: 'name', rad: Math.random() }, function (response) {
                            if (response.Result) {
                    ...
                    var onExpandEvent = function (event, treeId, treeNode) {
                        if (!treeNode.isExpand && treeNodeEnable.user) {
                            $.get("/platform/getOrgpersion?orgid=" + treeNode.ID + "&rd=" + Math.random(), {}, function (response) {
                                if (response.Result) {
                                    var treeObj = $.fn.zTree.getZTreeObj("cropOrgTree");
                                    var Members = response.ReturnObject;
                                    if (!isShowQuit)
                                        Members = Members.removeNodeByKeyValue("WorkType", "1"); /*删除WorkType为1为对象*/
                                    if (Members.length == 0) {
                                        ...
                                        for (var i = 0; i < Persons.length; i++) {
                                            var per = Persons[i];
                                            p_node.push({ "name": per.Name + (per.WorkType == 1 ? "(已离职)" : ""), "ID": per.ID, "nodeType": "user", "isManager": per.IsManager, "email": per.email });
                                        }
                    ...
                    $("#btn_addselecteditemforls", boxer_ctr).click(function () {
                        ...
                        if (recNodes.length > 0) {
                            ...
                            for (var i = 0; i < recNodes.length; i++) {
                                ...
                                var sp_sltItem = $('<span class="item_slted" tagid="' + recNodes[i].ID + '" tagname="' + recNodes[i].name + '" tagtype="' + recNodes[i].nodeType + '" email="' + recNodes[i].email + '"><em>' + _category + recNodes[i].name + '</em><a></a></span>');
    ...
    KSNSelect.prototype = {
        ...
        add: function (elem, txtData, nStart, nLen, guid) {//插入文本   参数  操作的元素，数据，起始坐标位置，用户输入字符长度
            ...
            if (this.seletedCallback != null)
                this.seletedCallback(guid, txtData);
            if (this.elem.setSelectionRange) {//W3C
                ...
                if (_this.timeout == 0 || curKeyCode == 13) {
                    ...
                    $.ajax({
                        ...
                        success: function (data) {
                            _this.timeout = 0;
                            if (data.Result) {
                                var tempdata = data.ReturnObject.user; //数据列表
                                var dataList = "";
                                ...
                                if (dataList.length > 0) {
                                    $("#" + _sugListID).show().html("<dl><dt>同事姓名、拼音、邮箱可检索</dt>" + dataList + "</dl>")
        ...
        inset: function () {//给dd绑定事件，
            ...
            $("#" + this.sugListID).delegate("dd", "click", function () {//事件委托
                if ($(this).index() == 0) {
                    ...
                    var txtData = $(this).find(".fw_agtlist_sname").text(); //
                    var guid = $(this).attr("data-uid");
                    _this.add(_this.elem, txtData, _this.at['pos'], _this.at['len'], guid)
                    _this.hiddenTip();
                }
            })
            ...
            switch (_key) {
                ...
                case 13:
                    ...
                    if (this.model == 0)//侃侃框
                    {
                        ...
                        //console.log('tag:' + txtData);
                        var uid = $("#" + eid).find(".on").attr("data-uid");
                        _this.add(_this.elem, txtData, _this.at['pos'], _this.at['len'], uid);
                    }
                    else {//选择用户、组织、群组
        ...
        ddtemplate: function (data, i) {
            ...
            if (data.datatype == "user") {
                ...
                _temp += "</div>";
                _temp += "<div class=\"fw_agtlist_ddright\"><span class=\"sgt_orgname\">" + data.name + "</span></div>";
                _temp += "</dd>";
                ...
                _temp += "</div>";
                _temp += "<div class=\"fw_agtlist_ddright\"><span class=\"sgt_groupname\">" + data.name + "</span></div>";
                _temp += "</dd>";
            }
            return _temp;
        },
        ...
        addin: function (uid, uname, uemail, addtype, datatype) {
            var _this = this;
            var go = true;
            ...
            if (this.model == 2 && go) {
                ...
                _thiselem.val("");
                _thiselem.before("<span class=\"fw_ksninput_slted\" data-uid=\"" + uid + "\" data-type=\"" + datatype + "\"><em>" + _category + uname + "</em>" + (_this.readonly ? "" : "<a></a>") + "</span>");
                _this.KInputReset();
        ...
        KIDataLoaded: function (data) {//原始数据加载【推荐使用KISetDataLoad()方法】
            ...
            if (data.length > 0) {
                ...
                for (var i = 0; i < data.length; i++) {
                    _this.addin(data[i].uid, data[i].uname, null, "LOAD_ADD"); /*LOAD_ADD:程序加载；SELECT_ADD:手动选择*/
                }
            }
        },
        ...
        KISetSelectedData: function (data, _callback) { /*设置或加载人、群组、组织，使之已选中【新增方法】*/
            ...
            if (data.length > 0) {
                ...
                $.post('/platform/getpers?', { data: loadinfos }, function (response) {
                    if (response) {
                        if (response.Result) {
                            var datalist = $.parseJSON(response.ReturnObject);
                            for (var i = 0; i < datalist.length; i++) {
                                _this.addin(datalist[i].id, datalist[i].name, null, "LOAD_ADD", datalist[i].type);
                            }
