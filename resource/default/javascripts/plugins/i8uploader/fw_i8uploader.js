...
define(function (require, exports) {
    ...
    exports.i8uploader = function (options) {
            ...
            if($.type(attachmentlist)=='array'&&attachmentlist.length>0){
                ...
                for(var i= 0,len=attachmentlist.length;i<len;i++){
                    ...
                    if(/(gif|jpg|jpeg|png|GIF|JPG|PNG)$/.test(attachmentlist[i].Extension)){
                        $li.find('.imgWrap').append('<img src="'+attachmentlist[i].ImageSmall+'" width="'+_options.thumbnailWidth+'" height="'+_options.thumbnailHeight+'" />');
                    }else{
        ...
        uploader.onUploadSuccess = function (file, response) {
            ...
            var currentLi = $('#' + file.id);
            if(response.Result){
                currentLi.data(response.ReturnObject);
                if (_options.uploadSuccess) {
                    _options.uploadSuccess(response.ReturnObject);
                }
            }
        ...
        uploader.onError = function (code) {
            switch (code) {
                ...
                case 'F_EXCEED_SIZE':alert('文件大小超限,单个文件大小限制为'+_options.fileSingleSizeLimit/(1024*1024)+'M,总文件大小限制为'+_options.fileSizeLimit/(1024*1024)+'M！'); break;
                case 'Q_EXCEED_NUM_LIMIT': alert('文件数超限,最多上传'+_options.fileNumLimit+'个文件'); break;
                case 'Q_EXCEED_SIZE_LIMIT': alert('文件大小超限,单个文件大小限制为'+_options.fileSingleSizeLimit/(1024*1024)+'M,总文件大小限制为'+_options.fileSizeLimit/(1024*1024)+'M！'); break;
